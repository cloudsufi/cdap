...
public class MessagingProgramStatePublisher implements ProgramStatePublisher {
  ...
  private MessagingProgramStatePublisher(MessagingService messagingService,
                                        String topicPrefix, int numTopics,
  ...
  private TopicId getTopic(Notification programStatusNotification) {
    String programRun = programStatusNotification.getProperties().get(ProgramOptionConstants.PROGRAM_RUN_ID);
    int topicNum = programRun == null ? 0 : (Math.abs(programRun.hashCode()) % topicIds.size());
    return  topicIds.get(topicNum);
  }
